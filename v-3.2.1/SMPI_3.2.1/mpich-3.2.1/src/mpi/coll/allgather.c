...
/* not declared static because a machine-specific function may call this 
   one in some cases */
#undef FUNCNAME
#define FUNCNAME MPIR_Allgather_intra_original
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Allgather_intra_original ( 
    const void *sendbuf,
    int sendcount,
    ...
    MPIR_Errflag_t *errflag )
{
    ...
    printf("Original Ring is called\n");
    fflush(stdout);
 // /*****************************************************HERE******************************************/     
...
#undef FUNCNAME
#define FUNCNAME MPIR_SEC_Allgather_intra
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_SEC_Allgather_intra ( 
    const void *sendbuf,
    int sendcount,
    MPI_Datatype sendtype,
    void *recvbuf,
    ...
    MPIR_Errflag_t *errflag )
{
  int mpi_errno = MPI_SUCCESS;
  if(SECURE_MPI){
    mpi_errno = MPIR_SEC_Allgather_intra(sendbuf, sendcount, sendtype,
                                         recvbuf, recvcount, recvtype,
                                         comm_ptr, errflag);
        if (mpi_errno) MPIR_ERR_POP(mpi_errno);
  }else{
    mpi_errno = MPIR_Allgather_intra_original(sendbuf, sendcount, sendtype,
                                         recvbuf, recvcount, recvtype,
                                         comm_ptr, errflag);
        if (mpi_errno)
         MPIR_ERR_POP(mpi_errno);
