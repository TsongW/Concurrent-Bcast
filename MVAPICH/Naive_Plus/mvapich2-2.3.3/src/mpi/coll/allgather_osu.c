            ...
            while (tmp_mask) {
                ...
                if ((dst > rank) && (rank < tree_root + nprocs_completed)
                    && (dst >= tree_root + nprocs_completed)) {
                        ...
                        // }//End Naive +
                            ...
                            in = (char*)((char*) recvbuf + offset);
                            out = (char*)((char*) ciphertext_recvbuf + (my_tree_root + mask) * (recvcount * recvtype_extent) + 16+12);
                            ...
                            MPIR_PVAR_INC(allgather, rd, send, in_size+16+12, MPI_CHAR); 
                            mpi_errno =
                                MPIC_Send(out,
                                            in_size+16+12, MPI_CHAR, dst,
                                            MPIR_ALLGATHER_TAG, comm_ptr, errflag);
                ...
                /* recv only if this proc. doesn't have data and sender
                    * has data */
                    ...
                    MPIR_PVAR_INC(allgather, rd, recv, ((comm_size - (my_tree_root + mask)) * recvcount * recvtype_extent) + 16 + 12, MPI_CHAR);
                    mpi_errno =
                        MPIC_Recv(((char *) ciphertext_recvbuf + offset + 16 + 12),
                                    (comm_size -
                                    (my_tree_root +
                                    mask)) * recvcount* recvtype_extent + 16+ 12, MPI_CHAR,
                                    dst, MPIR_ALLGATHER_TAG, comm_ptr, &status, errflag);
                    ...
                    * no. of processes whose data we don't have */
                    ...
                    in = (char*)((char*) ciphertext_recvbuf + offset + 16 + 12);
                    out = (char*)((char*) recvbuf + offset);
                          ...
                          MPIR_Errflag_t *errflag)
{
    ...
    rank = comm_ptr->rank;
    //    if(rank==0)
    // printf("MPIR_Allgather_NaivePlus_RDB_MV2\n");
                            ...
                            MPID_Comm * comm_ptr, MPIR_Errflag_t *errflag)
{
    ...
    int workload = (int) (p / shmem_leaders); //number of encryptions each leader has to perform 
    ...
    if(local_rank < shmem_leaders && security_approach==2){
        ...
        //void* out = (void*)((char*) ciphertext_shmem_buffer + local_rank * (workload * recvcnt * recvtype_extent + 12 + 16));
        void* out = (void*)((char*) ciphertext_shmem_buffer + (my_node * p + local_rank * workload)* recvcnt * recvtype_extent + (my_node * shmem_leaders + local_rank) * (16 + 12));
        RAND_bytes(out, 12); // 12 bytes of nonce

        unsigned long t=0;
        t = (unsigned long)(workload * recvcnt * recvtype_extent);
        if(local_rank == shmem_leaders-1){
            t += (p - (shmem_leaders * workload)) * recvcnt * recvtype_extent;
        }
        ...
        if(!EVP_AEAD_CTX_seal(ctx, out+12,  
                            &ciphertext_len, max_out_len,
                            out, 12,
                            shmem_buffer + ((my_node * p + local_rank * workload)* recvcnt * recvtype_extent),  t,
                            NULL, 0))
        {
    ...
    if (local_rank == 0 && (leader_comm_size > 1)) {
	    ...
	    //printf("Naive+\n");
            ...
            void* out = (void*)((char*) ciphertext_shmem_buffer + my_node * (p * recvcnt * recvtype_extent + shmem_leaders *(12 + 16)));
            unsigned long max_out_len = (p * recvcnt * recvtype_extent + shmem_leaders *(12 + 16));

            mpi_errno = MPIR_Allgather_impl(out, (max_out_len), MPI_CHAR,
                                            ciphertext_shmem_buffer, (max_out_len), MPI_CHAR,
...
#undef FUNCNAME
#define FUNCNAME MPIR_2lvl_SharedMem_Concurrent_Encryption_Allgather_MV2
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_2lvl_SharedMem_Concurrent_Encryption_Allgather_MV2(const void *sendbuf,int sendcnt, MPI_Datatype sendtype,
    ...
    if(security_approach == 2){
	  ...
	  (void*)((char*)shmem_buffer + (my_node * p + local_rank)), recvcnt, recvtype);*/
	if(local_rank == 0){
            
	    mpi_errno = MPIR_Gather_impl(in, 
					 recvcnt , recvtype,
					 (void*)((char*)shmem_buffer + (my_node * p * recvcnt * recvtype_extent)), 
					 recvcnt, recvtype,
					 0, shmem_commptr, errflag);
	...
	/*mpi_errno = MPIR_Barrier_impl(comm_ptr->node_comm, errflag);
        ...
        if(local_rank!=0){
            ...
            var=MPI_Type_size(sendtype, &sendtype_sz);
            var=MPI_Type_size(recvtype, &recvtype_sz);
            unsigned long   max_out_len = (unsigned long) (16 + (sendtype_sz*sendcnt));
            int i, next, dest;
            //printf("%d- local_rank:%d, leader_Comm_size: %d, rank/p:%d\n", rank, local_rank, leader_comm_size, (int)(rank/p));
            for( i = 0; i < n; i+=1){
                ...
                if(i != my_node){
                    ...
                    if(!EVP_AEAD_CTX_open(ctx, ((shmem_buffer+dest)),
                                    ...
                                    (ciphertext_shmem_buffer+next), 12,
                                    (ciphertext_shmem_buffer+next+12), (unsigned long )((recvcnt*recvtype_sz)+16),
                                    NULL, 0)){
...
#undef FUNCNAME
#define FUNCNAME MPIR_2lvl_Allgather_Encrypted_RDB_MV2
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
...
conf_check_end:
	...
	/***** Added by Mehran *****/
    if(MV2_Allgather_function == &MPIR_2lvl_Allgather_Multileader_Ring_MV2
