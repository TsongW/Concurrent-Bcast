...
static int MPIDU_Sched_start_entry(struct MPIDU_Sched *s, size_t idx, struct MPIDU_Sched_entry *e)
{
    ...
    switch (e->type) {
        ...
        case MPIDU_SCHED_ENTRY_BLOCKING_RECV:
            MPIU_DBG_MSG_D(COMM, VERBOSE, "starting BLOCKING_RECV entry %d\n", (int) idx);
            comm = e->u.recv.comm;
            ret_errno = MPIC_Recv(e->u.recv.buf, e->u.recv.count, e->u.recv.datatype,
                                   e->u.recv.src, s->tag, comm, e->u.recv.status, &r->errflag);
            
            
            if(ret_errno)
                printf("BLOCKING RECV FAILED!\n");
            /* Check if the error is actually fatal to the NBC or we can continue. */
            if (unlikely(ret_errno)) {
                if (MPIR_ERR_NONE == r->errflag) {
                    if (MPIX_ERR_PROC_FAILED == MPIR_ERR_GET_CLASS(ret_errno)) {
                ...
                * will be handled later in MPIDU_Sched_progress_state, so set to started here */
                e->status = MPIDU_SCHED_ENTRY_STATUS_STARTED;
                MPIU_DBG_MSG_D(COMM, VERBOSE, "Sched BLOCKING_RECV failed. Errflag: %d\n", (int) r->errflag);
            } else {
                e->status = MPIDU_SCHED_ENTRY_STATUS_STARTED;
            }
            break;
        ...
        case MPIDU_SCHED_ENTRY_CB:
            ...
            break;
        /******     Added by Mehran     *****/
        case MPIDU_SCHED_ENTRY_ENCRYPT:
            //Encrypt here
            
            MPIU_DBG_MSG_D(COMM, VERBOSE, "starting ENCRYPTION entry %d\n", (int) idx);


            RAND_bytes(e->u.crypt.outbuf , e->u.crypt.nonce_len);

            if(!EVP_AEAD_CTX_seal(ctx, e->u.crypt.outbuf+e->u.crypt.nonce_len,
                                    (e->u.crypt.out_len), e->u.crypt.max_out_len,
                                    e->u.crypt.outbuf, e->u.crypt.nonce_len,
                                    e->u.crypt.inbuf,  e->u.crypt.in_len,
                                    NULL, 0))
            {
                    printf("Error in encryption entry!!\n");
                    fflush(stdout);
                    goto fn_fail;
            }
            e->status = MPIDU_SCHED_ENTRY_STATUS_COMPLETE;
            break;
        case MPIDU_SCHED_ENTRY_DECRYPT:
            //decrypt here

            MPIU_DBG_MSG_D(COMM, VERBOSE, "starting DECRYPTION entry %d\n", (int) idx);

            if(!EVP_AEAD_CTX_open(ctx, e->u.crypt.outbuf,
                    e->u.crypt.out_len, e->u.crypt.max_out_len,
                    e->u.crypt.inbuf, e->u.crypt.nonce_len,
                    (e->u.crypt.inbuf+e->u.crypt.nonce_len), e->u.crypt.max_out_len,
                    NULL, 0)){
                        printf("Error in decryption entry!!\n");
                        fflush(stdout);
                        goto fn_fail;        
            }  
            break;
...
static int MPIDU_Sched_add_entry(struct MPIDU_Sched *s, int *idx, struct MPIDU_Sched_entry **e)
{
    ...
    if (s->num_entries == s->size) {
        ...
        if (entries == NULL) MPIR_ERR_SETANDJUMP(mpi_errno,MPI_ERR_OTHER,"**nomem");
...
#undef FUNCNAME
#define FUNCNAME MPID_Sched_brecv
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPID_Sched_brecv(void *buf, MPI_Aint count, MPI_Datatype datatype, int src, MPID_Comm *comm, MPI_Status *status, MPID_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;
    struct MPIDU_Sched_entry *e = NULL;
    ...
    e->type = MPIDU_SCHED_ENTRY_BLOCKING_RECV;
    e->status = MPIDU_SCHED_ENTRY_STATUS_NOT_STARTED;
    e->is_barrier = FALSE;
...
#undef FUNCNAME
#define FUNCNAME MPID_Sched_crypt
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
/* Schedules a cryptography operation (encryption or decryption) from "inbuf" to "outbuf".
 * The operation is specified with "opt" ('E' -> encryption and 'D' -> decryption ).
 */
int MPID_Sched_crypt(const void *inbuf, const MPI_Aint in_len, const MPI_Aint enc_overhead, const MPI_Aint nonce_len,
                    void *outbuf, MPI_Aint* out_len, const MPI_Aint max_out_len, const char opt, MPID_Sched_t s)

{
    int mpi_errno = MPI_SUCCESS;
    struct MPIDU_Sched_entry *e = NULL;
    struct MPIDU_Sched_crypt *crypt = NULL;

    mpi_errno = MPIDU_Sched_add_entry(s, NULL, &e);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);

    if (opt == 'E')
        e->type = MPIDU_SCHED_ENTRY_ENCRYPT;
    else if (opt == 'D')
        e->type = MPIDU_SCHED_ENTRY_DECRYPT;
    else
    {
        printf("Invalid Crypt Type!\n");
        goto fn_fail;
    }
    e->status = MPIDU_SCHED_ENTRY_STATUS_NOT_STARTED;
    e->is_barrier = FALSE;
    ...
    crypt->inbuf = inbuf;
    crypt->in_len = in_len;
    crypt->enc_overhead = enc_overhead;
    crypt->nonce_len = nonce_len;
    crypt->outbuf = outbuf;
    crypt->out_len = out_len;
    crypt->max_out_len = max_out_len;
