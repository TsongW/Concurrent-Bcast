											...
											MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
	...
	*/
	...
	context_id = (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) ?
	  ...
	  mpi_errno = MPID_Irecv((char*)recvbuf+recv_offset, enc_recv_size, MPI_CHAR, dst, MPIR_ALLGATHER_TAG,
	  ...
	  mpi_errno = MPID_Isend(sendbuf, curr_cnt, MPI_CHAR, dst, MPIR_ALLGATHER_TAG,
	  ...
	  *(&status) = (recv_req_ptr[0])->status;
	  ...
	  while (mask < comm_size) {
		...
		if (dst < comm_size) {
			...
			mpi_errno = MPID_Irecv((char*)recvbuf+recv_offset, (comm_size - dst_tree_root) *  enc_recv_size, MPI_CHAR, dst, MPIR_ALLGATHER_TAG,
			...
			mpi_errno = MPID_Isend(recvbuf + send_offset, curr_cnt, MPI_CHAR, dst, MPIR_ALLGATHER_TAG,
